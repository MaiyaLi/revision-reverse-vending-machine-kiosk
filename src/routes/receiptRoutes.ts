...
router.post('/send-email', async (req: Request, res: Response) => {
  try {
    ...
    if (!transactionId || !emailAddress) {
      return res.status(400).json({ success: false, error: 'transactionId and emailAddress are required' });
    }
    ...
    res.status(500).json({ success: false, error: error.message || 'Failed to send email receipt' });
  }
});
...
router.get('/:transactionId', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: error.message || 'Failed to get receipt' });
  }
});
